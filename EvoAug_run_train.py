...
def _run(rank, world_size, cfg):
    ...
    evo_aug = batch_apply_EvoAug.apply_EvoAug(augment_list, max_augs_per_seq=2, hard_aug=True)
    ...
    # I want to check how models will be loaded, will it be loaded only once for the entire training?
    mprint("Hey! Model loaded from meta_dir!")
    ...
    while state['step'] < num_train_steps + 1:
        ...
        # for EvoAug, augment this batch's inputs with the prepared class
        augment_inputs = evo_aug(inputs)
        # evoaug_comparison = augment_inputs == inputs
        # num_different_rows = (evoaug_comparison.all(dim=1) == False).sum().item()
        # mprint("step: %d, mutated sequences: %s" % (step, str(num_different_rows)))
        ...
        inputs, target = augment_inputs.to(device), target.to(device)
